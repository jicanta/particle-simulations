...
const std::string& Arguments::command() const {
    return command_;
}
...
std::string Arguments::text(const std::string& name, const std::string& fallback) const {
...
unsigned int Arguments::unsignedInteger(const std::string& name, unsigned int fallback) const {
    return has(name) ? static_cast<unsigned int>(std::stoul(valueOf(name))) : fallback;
}

std::string Arguments::requiredText(const std::string& name) const {
    return valueOf(name);
}
